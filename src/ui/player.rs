...
impl PlayerState {
    pub fn new() -> Self {
        ...
        // Initialize mpv with more robust error handling
        ...
        match Mpv::new() {
            Ok(mpv) => {
                ...
                match mpv.get_property::<String>("mpv-version") {
                    ...
                    Err(_) => debug!("Could not get MPV version"),
                }
                
                // Configure mpv for audio-only playback
                if let Err(e) = mpv.set_property("video", false) {
                    warn!("Failed to disable video: {}", e);
                } else {
                    debug!("Video disabled successfully");
                }
                
                if let Err(e) = mpv.set_property("vo", "null") {
                    warn!("Failed to set video output to null: {}", e);
                } else {
                    debug!("Video output set to null successfully");
                }
                
                // Set some additional audio-focused properties
                if let Err(e) = mpv.set_property("terminal", true) {
                    warn!("Failed to enable terminal mode: {}", e);
                } else {
                    debug!("Terminal mode enabled");
                }
                
                if let Err(e) = mpv.set_property("msg-level", "all=info") {
                    warn!("Failed to set msg-level: {}", e);
                } else {
                    debug!("MPV msg-level set to info");
                }
                
                // Test basic MPV functionality
                match mpv.get_property::<String>("audio-device") {
                    Ok(device) => info!("Audio device: {}", device),
                    Err(e) => warn!("Could not get audio device: {}", e),
                }
                
                state.mpv = Some(mpv);
                info!("MPV configuration completed - audio playback available");
            }
            Err(e) => {
                match e {
                    libmpv::Error::VersionMismatch { linked, loaded } => {
                        error!("MPV version mismatch - linked: {}, loaded: {}", linked, loaded);
                        error!("This usually means the libmpv version at compile time doesn't match runtime");
                        error!("Try rebuilding with: cargo clean && cargo build");
                    }
                    _ => {
                ...
                info!("The application will still work for browsing music and displaying metadata");
                // Continue without MPV - the UI will still work
            }
        }
